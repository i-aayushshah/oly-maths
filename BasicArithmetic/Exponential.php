...
body {
  ...
  <body>
    ...
    <h4>This interface is provided for raising an arbitrary precision number to the power of another.</h4>
    <div>
      <label for="numbers">Enter two numbers separated by a comma (first the base, then the exponent):</label>
      <input type="text" id="numbers" onkeydown="handleEnterKey(event)">
    </div>
    <div>
      <label for="decimal-places">Enter number of decimal places:</label>
      <input type="number" id="decimal-places" min="0" onkeydown="handleEnterKey(event)">
    </div>
    <div>
  ...
  <script>
    function calculatePower() {
      ...
      var decimalPlacesInput = document.getElementById("decimal-places");
      var resultOutput = document.getElementById("result");

      var inputValues = numbersInput.value.split(",");
      var base = parseFloat(inputValues[0]);
      var exponent = parseFloat(inputValues[1]);
      var decimalPlaces = parseInt(decimalPlacesInput.value);

      if (isNaN(base) || isNaN(exponent)) {
        resultOutput.textContent = "Please enter valid numbers for the base and exponent.";
        return;
      }

      var result = Math.pow(base, exponent);
      resultOutput.textContent = "Result: " + result.toFixed(decimalPlaces);
    }
