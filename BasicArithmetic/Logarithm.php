...
body {
  ...
  <body>
    ...
    <div class="container">
    <h1>Logarithms</h1>
    <h4>This interface is provided for calculating the logarithms numbers.</h4>
    <div>
      <label for="numbers">Enter two numbers separated by a comma (first the number, then the base):</label>
      <input type="text" id="numbers" onkeydown="handleEnterKey(event)">
    </div>
    <div>
      <button onclick="calculateLogarithm()">Calculate</button>
  ...
  <script>
    function calculateLogarithm() {
      ...
      var number = parseFloat(inputValues[0]);
      var base = inputValues[1];

      if (isNaN(number) || (base !== "e" && isNaN(parseFloat(base)))) {
        resultOutput.textContent = "Please enter a valid number and base.";
        return;
      }

      var logarithm = Math.log(number) / (base === "e" ? 1 : Math.log(parseFloat(base)));
      resultOutput.textContent = "Logarithm: " + logarithm;
    }
