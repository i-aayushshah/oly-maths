...
body {
  ...
  <body>
    ...
    <div class="container">
    <h1>Modulo</h1>
    <h4>This interface is provided for calculating the modulo of an arbitrary precision integer. Or simply remainder.</h4>
    <div>
      <label for="numbers">Enter two positive integers separated by a comma (numerator and denominator):</label>
      <input type="text" id="numbers" onkeydown="handleEnterKey(event)">
    </div>
    <div>
      <button onclick="calculateModulo()">Calculate</button>
  ...
  <script>
    function calculateModulo() {
      ...
      var dividend = parseInt(inputValues[0]);
      var divisor = parseInt(inputValues[1]);

      if (isNaN(dividend) || isNaN(divisor) || dividend < 0 || divisor <= 0) {
        resultOutput.textContent = "Please enter two valid positive integers.";
        return;
      }

      var modulo = dividend % divisor;
      resultOutput.textContent = "Modulo: " + modulo;
    }
