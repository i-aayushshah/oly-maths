...
body {
  ...
  <body>
    ...
    <div>
      <label for="terms">Enter the first term (a), number of terms (n), and common ratio (r) separated by commas:</label>
  ...
  <script>
    function generateProgression() {
      ...
      var firstTerm = parseFloat(inputValues[0]);
      var numTerms = parseInt(inputValues[1]);
      var commonRatio = parseFloat(inputValues[2]);

      if (isNaN(firstTerm) || isNaN(numTerms) || isNaN(commonRatio)) {
        resultOutput.textContent = "Please enter valid numbers for the first term, number of terms, and common ratio.";
        return;
      }

      var progression = [];
      var nthTerm = firstTerm * Math.pow(commonRatio, numTerms - 1);

      for (var i = 0; i < numTerms; i++) {
        var term = firstTerm * Math.pow(commonRatio, i);
