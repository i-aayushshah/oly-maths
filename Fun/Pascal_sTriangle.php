...
body {
  ...
  <body>
    ...
    <h4>This interface is privided for generating Pascal's Triangle.</h4>
    <div>
    ...
    <div>
      ...
      <button class="btn-clear" onclick="clearTriangle()">Clear</button>
    </div>
  ...
  <script>
    function generateTriangle() {
      ...
      var triangleOutput = document.getElementById("triangle");

      triangleOutput.innerHTML = ""; // Clear previous triangle

      var numRows = parseInt(rowsInput.value);

      if (isNaN(numRows) || numRows <= 0) {
        triangleOutput.textContent = "Please enter a valid number of rows greater than zero.";
        return;
      }
      ...
      for (var i = 0; i < triangle.length; i++) {
        ...
        for (var j = 0; j < triangle[i].length; j++) {
          rowText += triangle[i][j] + " ";
        }

        triangleText += rowText.trim() + "<br>";
      }

      triangleOutput.innerHTML = triangleText;
    }
