...
body {
  ...
  <body>
    ...
    <h4>This interface is provided for showing the steps in getting to the Kaprekar's Constant.</h4>
    <div>
      <label for="number">Enter a four-digit number. Leading zeros are not allowed.</label>
  ...
  <script>
    function calculateKaprekar() {
      ...
      if (!/^[1-9]\d{3}$/.test(number)) {
        errorOutput.textContent = "Enter a 4-digit positive number.";
        resultOutput.textContent = "";
        return;
      }

      var steps = getKaprekarSteps(number);
      var output = "Steps to reach Kaprekar's Constant (6174):<br>";
      output += steps.join("<br>");
    ...
    function getKaprekarSteps(number) {
      var steps = [];

      while (number !== "6174") {
        var asc = number.toString().split("").sort().join("");
        var desc = asc.split("").reverse().join("");

        var diff = desc - asc;
        steps.push(desc + " - " + asc + " = " + diff);
        number = diff.toString().padStart(4, "0");
      }
