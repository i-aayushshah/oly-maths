...
body {
  ...
  <body>
    ...
    <section>
    <div class="container">
    <h1>Binary Quadratic Forms - Properties</h1>
    <h4>Returns some common properties of a binary quadratic forms like the discriminant, determinant, reduction status(for both -ve and +ve discriminants) etc.</h4>
    <div>
      <label for="coefficients">Enter the coefficients 
        a
        ,
        b
        ,
      ...
      <textarea id="coefficients"></textarea>
    </div>
    <div>
  ...
  <script>
    function calculateProperties() {
      var coefficientsInput = document.getElementById("coefficients");
      var resultOutput = document.getElementById("result");

      var coefficients = coefficientsInput.value.split(",").map(function (coefficient) {
        return parseInt(coefficient.trim());
      });

      var properties = calculateBinaryQuadraticFormProperties(coefficients);

      var output = "Discriminant: " + properties.discriminant + "<br><br>";
      output += "Determinant: " + properties.determinant + "<br><br>";
      output += "Reduction Status (for -ve discriminant): " + properties.negativeReductionStatus + "<br><br>";
      output += "Reduction Status (for +ve discriminant): " + properties.positiveReductionStatus;

      resultOutput.innerHTML = output;
    }

    function calculateBinaryQuadraticFormProperties(coefficients) {
      ...
      var negativeReductionStatus = calculateReductionStatus(a, b, c, -1);
      var positiveReductionStatus = calculateReductionStatus(a, b, c, 1);

      return {
        discriminant: discriminant,
        determinant: determinant,
        negativeReductionStatus: negativeReductionStatus,
        positiveReductionStatus: positiveReductionStatus
      };
    }
    ...
    function calculateReductionStatus(a, b, c, sign) {
      var reductionStatus = "";
