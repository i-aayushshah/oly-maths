...
body {
  ...
  <body>
        ...
        <ul class="menu-items">
          ...
          <li class="item">
            ...
            <ul class="menu-items submenu">
              ...
              <li class="item">
                <a href="/WebProject/BasicArithmetic/multiply.php">Multiply</a>
              </li>
              <li class="item">
    ...
    <h4>This solves for x and y in ax + by = c, where a, b, c are arbitrary precision integers.
        c can be negative, but a and b should be positive.</h4>
    <div>
      <label for="equation">Enter the coefficients a, b, and c (separated by commas):</label>
      <input type="text" id="equation" onkeydown="handleEnterKey(event)">
    </div>
    <div>
...
function solveEquation() {
  ...
  var equationValues = equationInput.value.split(",");
  var a = parseInt(equationValues[0]);
  var b = parseInt(equationValues[1]);
  var c = parseInt(equationValues[2]);

  // Check if a and b are positive
  if (a <= 0 || b <= 0) {
    resultOutput.textContent = "Coefficients a and b must be positive.";
    return;
  }

  var solutions = findSolutions(a, b, c);

  displaySolutions(solutions, a, b, c);
}

    function findSolutions(a, b, c) {
      var solutions = [];
      var gcd = extendedEuclideanAlgorithm(a, b);
      ...
      extendedEuclideanAlgorithm(a, b, function (x, y) {
        x0 = x;
        y0 = y;
      });
      ...
      while (k <= 10) {
        var xSolution = x + (k * (b / gcd));
        var ySolution = y - (k * (a / gcd));
        solutions.push({ k: k, x: xSolution, y: ySolution });
        k++;
      }
    ...
    function extendedEuclideanAlgorithm(a, b, callback) {
      var x0 = 1;
      var y0 = 0;
      var x1 = 0;
      var y1 = 1;
    ...
    function displaySolutions(solutions, a, b, c) {
      var resultOutput = document.getElementById("result");

      if (solutions.length === 0) {
        resultOutput.textContent = "No solutions found.";
        return;
      }
      ...
      var kHeader = document.createElement("th");
      kHeader.textContent = "k";
      ...
      resultOutput.appendChild(document.createTextNode("Input equation: " + a + "x + " + b + "y = " + c));
      resultOutput.appendChild(document.createElement("br"));
      resultOutput.appendChild(document.createTextNode("Here are some specific solutions to the equation:"));
