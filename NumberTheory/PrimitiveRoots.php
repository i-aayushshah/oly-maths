...
body {
  ...
  <body>
    ...
    <section>
    <div class="container">
    <h1>Primitive Roots Calculator</h1>
    <h4>Returns the primitive root(s) of a number n, which is equivalent to finding the generator(s) of the multiplicative group (Z/nZ)×. Works for both prime numbers and composites with primitive roots.</h4>
    <div>
      <label for="number">Enter a positive integer (n):</label>
      <input type="text" id="number" onkeydown="handleEnterKey(event)">
    </div>
    <div>
      <button onclick="calculatePrimitiveRoots()">Calculate</button>
  ...
  <script>
    function calculatePrimitiveRoots() {
      ...
      if (isNaN(n) || n <= 0) {
        resultOutput.textContent = "Please enter a valid positive integer (n).";
        return;
      }

      var primitiveRoots = findPrimitiveRoots(n);
      displayPrimitiveRoots(primitiveRoots);
    }

    function findPrimitiveRoots(n) {
      var primitiveRoots = [];
      var eulerPhi = calculateEulerPhi(n);

      for (var i = 1; i < n; i++) {
        if (isPrimitiveRoot(i, eulerPhi, n)) {
    ...
    function isPrimitiveRoot(a, eulerPhi, n) {
      if (gcd(a, n) !== 1) {
        return false;
      }

      var factors = getPrimeFactors(eulerPhi);

      for (var i = 0; i < factors.length; i++) {
        if (fastModularExponentiation(a, eulerPhi / factors[i], n) === 1) {
          return false;
        }
      }
    ...
    function displayPrimitiveRoots(primitiveRoots) {
      var resultOutput = document.getElementById("result");

      resultOutput.innerHTML = "";
      resultOutput.appendChild(document.createTextNode("Number of primitive roots of " + primitiveRoots.length + ":"));
      resultOutput.appendChild(document.createElement("br"));
      resultOutput.appendChild(document.createTextNode("Primitive Root(s): " + primitiveRoots.join(", ")));
    }
