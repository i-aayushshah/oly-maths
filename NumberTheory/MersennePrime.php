...
body {
  ...
  <body>
    ...
    <h4>Generate and check Mersenne primes. Enter an exponent, and the tool will determine if 2^exponent - 1 is a Mersenne prime and calculate its value.</h4>
    <div>
      <label for="exponent">Enter an exponent:</label>
      <input type="text" id="exponent" onkeydown="handleEnterKey(event)">
    </div>
    <div>
      <button onclick="checkMersennePrime()">Check</button>
  ...
  <script>
    function checkMersennePrime() {
      ...
      if (isNaN(exponent) || exponent <= 0) {
        resultOutput.textContent = "Please enter a valid positive integer for the exponent.";
        return;
      }

      var mersennePrime = calculateMersennePrime(exponent);

      if (isMersennePrime(mersennePrime)) {
        resultOutput.innerHTML = "<strong>2^" + exponent + " - 1 is a Mersenne prime.</strong><br><br>";
        resultOutput.innerHTML += "The value of the Mersenne prime is " + mersennePrime + ".";
      } else {
        resultOutput.textContent = "2^" + exponent + " - 1 is not a Mersenne prime.";
      }
    }

    function calculateMersennePrime(exponent) {
      var result = Math.pow(2, exponent) - 1;
      return result;
    }
    ...
    function isMersennePrime(number) {
      ...
      var exponent = 1;
      var mersenneNumber = 1;

      while (mersenneNumber < number) {
        exponent++;
        mersenneNumber = Math.pow(2, exponent) - 1;
      }

      return mersenneNumber === number;
    }
