...
body {
  ...
  <body>
    ...
    <section>
    <div class="container">
    <h1>Wilson's Theorem Checker</h1>
    <h4>Check if a number satisfies Wilson's theorem. Enter a number, and the tool will determine if it is a prime number based on Wilson's theorem.</h4>
    <div>
      <label for="number">Enter a number:</label>
      <input type="text" id="number" onkeydown="handleEnterKey(event)">
    </div>
    <div>
      <button onclick="checkWilsonTheorem()">Check</button>
  ...
  <script>
    function checkWilsonTheorem() {
      ...
      if (isNaN(number) || number <= 0) {
        resultOutput.textContent = "Please enter a valid positive integer.";
        return;
      }

      var isPrime = checkPrimeUsingWilsonTheorem(number);

      if (isPrime) {
        var wilsonResult = (factorial(number - 1) + 1) % (number * number) === 0;

        if (wilsonResult) {
          resultOutput.textContent = number + " satisfies Wilson's theorem and is a prime number.";
        } else {
          resultOutput.textContent = number + " is a prime number, but does not satisfy Wilson's theorem.";
        }
      } else {
        resultOutput.textContent = number + " is not a prime number.";
      }
    }

    function checkPrimeUsingWilsonTheorem(number) {
      if (number < 2) {
        return false;
      }
