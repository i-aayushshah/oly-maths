...
body {
  ...
  <body>
    ...
    <section>
    <div class="container">
    <h1>Twin Prime Search</h1>
    <h4>Twin primes are prime numbers that differ by 2, such as (3, 5), (11, 13), (17, 19), and so on. The calculator can display a list of twin primes found within the given range.</h4>
    <div>
      <label for="start">Start number:</label>
      <input type="number" id="start" min="1" onkeydown="handleEnterKey(event)">
    </div>
    <div>
      <label for="end">End number:</label>
      <input type="number" id="end" min="1" onkeydown="handleEnterKey(event)">
    </div>
    <div>
      <button onclick="searchTwinPrimes()">Search</button>
  ...
  <script>
    function searchTwinPrimes() {
      ...
      if (isNaN(start) || isNaN(end) || start < 1 || end < 1 || start > end) {
        resultOutput.textContent = "Please enter valid start and end numbers.";
        return;
      }

      var twinPrimes = getTwinPrimes(start, end);

      if (twinPrimes.length === 0) {
        resultOutput.textContent = "No twin prime numbers found within this range.";
      } else {
        resultOutput.textContent = "Twin prime numbers within this range are: " + twinPrimes.join(", ");
      }
    }

    function getTwinPrimes(start, end) {
      var twinPrimes = [];
