...
body {
  ...
  <body>
    ...
    <div class="container">
    <h1>Primorial Calculation</h1>
    <h4>This calculator computes the primorial of a number. The primorial of a number is the product of all prime numbers less than or equal to that number.</h4>
    <div>
      <label for="number">Enter a number:</label>
      <input type="number" id="number" min="1" onkeydown="handleEnterKey(event)">
    </div>
    <div>
  ...
  <script>
    function calculatePrimorial() {
      ...
      if (isNaN(number) || number < 1) {
        resultOutput.textContent = "Please enter a valid number greater than 0.";
        return;
      }

      var primorial = calculatePrimorialNumber(number);
      resultOutput.textContent = "The primorial of " + number + " (" + number + "#) is: " + primorial;
    }

    function calculatePrimorialNumber(number) {
      var primes = [];
      var product = 1;
