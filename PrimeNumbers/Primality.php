...
body {
  ...
  <body>
    ...
    <h4>Tests if entered number is prime or not. It uses Miller-Rabin Primality Test, and returns "Yes" if prime, returns "No" if not:</h4>
    <div>
      <label for="number">Enter a positive number.</label>
      <input type="number" id="number" onkeydown="handleEnterKey(event)">
    </div>
    <div>
  ...
  <script>
    function testPrimality() {
      ...
      if (number < 2) {
        resultOutput.textContent = "No";
        return;
      }

      if (isPrime(number)) {
        resultOutput.textContent = "Yes";
        resultOutput.classList.remove("result-no");
        resultOutput.classList.remove("result-maybe");
        resultOutput.classList.add("result-yes");
      } else {
        resultOutput.textContent = "No";
        resultOutput.classList.remove("result-yes");
        resultOutput.classList.remove("result-maybe");
        resultOutput.classList.add("result-no");
      }
    }
