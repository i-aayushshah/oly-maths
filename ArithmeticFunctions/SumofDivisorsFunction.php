...
body {
  ...
  <body>
    ...
    <h4>This returns the value of 
        σ
        k
        (n) , where 
        σ
        k
        (n) denotes the sum of the kth powers of the positive divisors of the positive integer n. The divisors include 1 and n itself.</h4>
    <div>
      <label for="input">Enter two whole numbers seperated by comma, first n, then k. k can be 0, in that case, the function returns a count of the number of divisors of n.:</label>
      <input type="text" id="input" placeholder="Enter n, k (e.g., 10, 2)">
    </div>
    <div>
      <button onclick="calculateSumOfDivisors()">Calculate</button>
  ...
  <script>
    ...
    // Get the input field and result output elements
    ...
    function calculateSumOfDivisors() {
      var input = inputField.value;
      var values = input.split(",");

      if (values.length !== 2) {
        resultOutput.textContent = "Please enter two whole numbers separated by a comma.";
        return;
      }

      var n = parseInt(values[0].trim());
      var k = parseInt(values[1].trim());

      if (isNaN(n) || isNaN(k)) {
        resultOutput.textContent = "Please enter valid whole numbers for n and k.";
        return;
      }

      var result = sumOfDivisors(n, k);

      resultOutput.textContent = "σ" + k + "(" + n + ") = " + result;
    }
    ...
    function sumOfDivisors(n, k) {
      if (k === 0) {
        ...
        // Calculate the sum of kth powers of divisors
        var sum = 0;
        for (var j = 1; j <= n; j++) {
          if (n % j === 0) {
            sum += Math.pow(j, k);
