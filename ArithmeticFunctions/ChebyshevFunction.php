...
body {
  ...
  <body>
    ...
    <div class="container">
    <h1>Chebyshev Function Calculator</h1>
    <h4>The Chebyshev function (ψ(x)) counts the number of prime numbers less than or equal to x. It is an arithmetic function named after the Russian mathematician Pafnuty Chebyshev. It is defined for a positive real number x as follows:

        ϑ(x) = ∑ p ≤ x log(p)

        where the sum is taken over all prime numbers p less than or equal to x, and log(p) denotes the natural logarithm of p.</h4>
    <div>
      <label for="number">Enter a positive integer (x):</label>
      <input type="number" id="number" onkeypress="handleKeyPress(event)">
    </div>
    <div>
      <button onclick="calculateChebyshev()">Calculate</button>
  ...
  <script>
    function calculateChebyshev() {
      ...
      if (isNaN(x) || x <= 0) {
        resultOutput.textContent = "Please enter a valid positive integer (x).";
        return;
      }

      var chebyshev = countPrimes(x);
      displayChebyshevValue(x, chebyshev);
    }

    function countPrimes(x) {
      var primes = [];
      var isPrime = new Array(x + 1).fill(true);
    ...
    function displayChebyshevValue(x, chebyshev) {
  var resultOutput = document.getElementById("result");
  resultOutput.textContent = "ψ(" + x + ") = " + chebyshev;
}
