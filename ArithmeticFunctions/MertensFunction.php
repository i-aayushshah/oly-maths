...
body {
  ...
  <body>
    ...
    <section>
    <div class="container">
    <h1>Mertens Function Calculator</h1>
    <h4>Mertens function is an arithmetic function that measures the partial sums of the Möbius function. It is denoted by: M(n) = ∑μ(k) for k = 1 to n, where μ(k) is the Möbius function.</h4>
    <div>
      <label for="number">Enter a positive integer (n):</label>
      <input type="number" id="number" onkeypress="handleKeyPress(event)">
    </div>
    <div>
      <button onclick="calculateMertens()">Calculate</button>
  ...
  <script>
    function calculateMertens() {
      ...
      var mertens = calculateMertensValues(n);
      displayMertensValues(mertens);
    }

    function calculateMertensValues(n) {
      var mertens = new Array(n + 1);
      mertens[1] = 1;
    ...
    function mobius(n) {
      ...
      var uniqueFactors = Array.from(new Set(factors));
    ...
    function displayMertensValues(mertens) {
      var resultOutput = document.getElementById("result");
      var output = "";

      output += "Mertens function values up to " + (mertens.length - 1) + ":<br>";

      for (var i = 1; i < mertens.length; i++) {
        output += "M(" + i + ") = " + mertens[i] + "<br>";
      }
