...
body {
  ...
  <body>
    ...
    <div>
      <button onclick="calculateCarmichaelLambda()">Calculate</button>
  ...
  <script>
    function calculateCarmichaelLambda() {
      ...
      var lambdaValue = carmichaelLambda(n);
      displayResult(n, lambdaValue);
    }
    ...
    function carmichaelLambda(n) {
      ...
      var distinctFactors = Array.from(new Set(factors));

      if (distinctFactors.length === 1) {
        // If n is a prime power p^k, then λ(n) = φ(n)
        return eulerTotient(n);
      }

      var result = 1;
      for (var i = 0; i < distinctFactors.length; i++) {
        var p = distinctFactors[i];
    ...
    function displayResult(n, lambdaValue) {
      var resultOutput = document.getElementById("result");

      resultOutput.innerHTML = "";
      resultOutput.appendChild(document.createTextNode("Carmichael Lambda Function (λ):"));
      resultOutput.appendChild(document.createElement("br"));
      resultOutput.appendChild(document.createTextNode("λ(" + n + ") = " + lambdaValue));
    }
