...
body {
  ...
  <body>
    ...
    <section>
    <div class="container">
    <h1>Polynomial Multiplication Calculator</h1>
    <div>
      <label for="polynomials">Enter the polynomials to be multiplied separated by semicolons (;): [Represent a polynomial by it's coefficients in descending order of powers.]</label>
      <textarea id="polynomials"></textarea>
    </div>
    <div>
      <button onclick="calculatePolynomialMultiplication()">Calculate</button>
  ...
  <script>
    function calculatePolynomialMultiplication() {
      var polynomialsInput = document.getElementById("polynomials");
      var resultOutput = document.getElementById("result");

      var polynomials = polynomialsInput.value.trim().split(";");

      if (polynomials.length < 2) {
        resultOutput.textContent = "Please enter at least two polynomials.";
        return;
      }

      var product = multiplyPolynomials(polynomials);
      var inputPolynomials = formatInputPolynomials(polynomials);
      var productStr = formatPolynomial(product);

      resultOutput.innerHTML = "Input Polynomials:<br>" + inputPolynomials + "<br><br>Polynomial Product:<br>" + productStr;
    }
    ...
    function multiplyPolynomials(polynomials) {
      ...
      for (var i = 0; i < polynomials.length; i++) {
        var polynomial = parsePolynomial(polynomials[i]);

        if (polynomial.length > 0) {
          if (product.length === 0) {
            product = polynomial;
          } else {
            product = multiplyTwoPolynomials(product, polynomial);
          }
        }
      }
    ...
    function parsePolynomial(polynomialStr) {
      var coefficients = polynomialStr.split(",").map(Number);
    ...
    function formatInputPolynomials(polynomials) {
      var inputPolynomials = "";

      for (var i = 0; i < polynomials.length; i++) {
        var polynomial = parsePolynomial(polynomials[i]);
        var polynomialStr = formatPolynomial(polynomial);

        inputPolynomials += polynomialStr;
      ...
      for (var i = 0; i < polynomial.length; i++) {
        ...
        if (term !== 0) {
          ...
          if (power === 0) {
            ...
            polynomialStr += term + "x";
          } else {
            polynomialStr += term + "x^" + power;
          }
        }
      }
